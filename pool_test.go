...
func TestPool_NewPool(t *testing.T) {
	pool, _ := NewPool(testFactory(), testConnTestFunc())
	defer pool.Close()
	var (
		conn1 io.Closer
		conn2 io.Closer
	...
	pool, _ = NewPool(testFactory(), testConnTestFunc(), OptionMaxOpen(3))
...
func TestPool_Pressure(t *testing.T) {
	pool, _ := NewPool(testFactory(), testConnTestFunc(), OptionMaxOpen(10))
...
func TestPool_Acquire(t *testing.T) {
	pool, _ := NewPool(testConnFailFactory(), testConnTestFunc(), OptionMaxOpen(2))
...
func TestPool_Expire(t *testing.T) {
	pool, _ := NewPool(testFactory(), testConnTestFunc(), OptionMaxOpen(1), OptionConnMaxAliveTime(time.Second))
